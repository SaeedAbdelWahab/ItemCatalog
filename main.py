...
def catalogHome():
    ...
    for i in range(5):
        dictionary[items[i].name] = \
            dbsession.query(Category)\
            .filter_by(id=items[i].category_id).first()
...
def categoryItems(categoryName):
    dbsession = DBSession()
    category = \
        dbsession.query(Category).filter_by(name=categoryName).first()
...
def getItem(categoryName, itemName):
    ...
    category = \
        dbsession.query(Category).filter_by(name=categoryName).first()
    item = dbsession.query(Item).filter_by(name=itemName,
...
def editItem(categoryName, itemName):
    if session['logged_in']:
        ...
        category = \
            dbsession.query(Category).filter_by(name=categoryName).first()
        editedItem = dbsession.query(Item).filter_by(name=itemName,
                                                     category_id=category.id)\
            .first()
        if request.method == 'POST':
            ...
            return render_template('editedItem.html',
...
def deleteItem(categoryName, itemName):
    if session['logged_in']:
        ...
        category = \
            dbsession.query(Category).filter_by(name=categoryName).first()
        itemToDelete = dbsession.query(Item)\
            .filter_by(name=itemName,
                       category_id=category.id).first()
        if request.method == 'POST':
